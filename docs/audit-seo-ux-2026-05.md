...
## État d'avancement — 2026-05-23

> Mise à jour quotidienne à la clôture de chaque PR audit. Le tableau
> ci-dessous reflète l'historique git réel (commits cités à droite).
> **Référence canonique** : §7 _Plan d'action priorisé_ plus bas.

**Score global** : **7/15 items faits** · 2 partiels · 6 restants.

### Items faits (7)

| #   | Item                                                | Commit / preuve                                                            |
| --- | --------------------------------------------------- | -------------------------------------------------------------------------- |
| 1   | Hreflang fr-CI / fr-FR / x-default + canonical      | `6963a63` _feat(seo): 4 quick wins audit P2 §7_                            |
| 2   | OG image dynamique par page                         | `6963a63` + `feat/p12-og-articles`                                         |
| 3   | Schema.org Article enrichi sur `/insights/[slug]`   | `6963a63`                                                                  |
| 4   | BreadcrumbList généralisé                           | `6963a63` + `1cd478a` _ui-polish-v2 (4 pages détail)_                      |
| 5   | Lighthouse + axe-core en CI (fail < 95)             | `3ec46b5` _feat(p17-qa-final)_ + `fd2f155` (Lighthouse perf 0.95 atteinte) |
| 6   | Page `/a-propos/equipe` premium + Person schema     | `fd2f155` _feat(seo): équipe + Lighthouse_                                 |
| 10  | **Cmd+K public** (palette + index 38 entrées + a11) | `763b723` _feat(p2): Cmd+K command palette publique (audit §7 #10)_        |

### Items partiels (2)

| #   | Item                                        | État                                                                                                                                                                                        |
| --- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | 10 articles fondateurs signés Debora Ahouma | **4/10** dans le fallback hardcodé `lib/articles.ts`. Payload pas seedé avec corpus complet. **Reste 6 articles à rédiger + seed**.                                                         |
| 11  | 3 papers `/laboratoire/publications/[slug]` | Catalogue de 15 entrées dans `lib/data/laboratoire.ts` mais **pas de pages détail par paper, pas de PDF téléchargeables, pas de schema TechArticle**. Reste : 3 landings + 3 PDFs + schema. |

### Items restants (6)

| #   | Item                                                                                         | Sprint cible                    | Score audit (impact/effort) |
| --- | -------------------------------------------------------------------------------------------- | ------------------------------- | --------------------------- |
| 15  | **Whitepaper « IA souveraine en CI 2026 » téléchargeable contre email**                      | **P2 (en cours, prochaine PR)** | 5 / 2 → 2.50                |
| 8   | Mockups produits réels (remplacer `MediaPlaceholder` — voir `docs/captures-todo-2026-05.md`) | P3                              | 5 / 4 → 1.25                |
| 9   | Mega-menu navbar (Solutions / Expertises / Livre / Laboratoire)                              | P3                              | 4 / 2 → 2.00                |
| 13  | Pages géo `/abidjan`, `/uemoa`, `/cocody`                                                    | P3                              | 4 / 2 → 2.00                |
| 14  | Questionnaire interactif `/audit-ia` (5 questions → reco contextuelle)                       | P4                              | 4 / 3 → 1.33                |
| 12  | Témoignages clients vidéo + section « Résultats » sur pages produits                         | P5                              | 5 / 4 → 1.25                |

### Gouvernance

- Audit créé par `8fd3a42` (commit qui a accidentellement été poussé sur `main` via PR #46 sans redescente dans `develop`). Restauration dans `develop` via la présente PR `docs/restore-audit-p2-seo-ux`.
- **Référencé depuis** : [`README.md`](../README.md#documentation--audits), [`CLAUDE.md`](../CLAUDE.md) QUICK REFERENCE, [`docs/README.md`](./README.md).
